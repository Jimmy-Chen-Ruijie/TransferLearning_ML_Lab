...
if __name__ == "__main__":
    ...
    if opt.modularization == 1:
        ...
        input_nc = int(parse_model_config(opt.models_def[1])[0]['channels'])
        ...
        input_nc_1 = int(parse_model_config(opt.models_def[1])[0]['channels'])
        input_nc_2 = int(parse_model_config(opt.models_def[2])[0]['channels'])
        input_nc_3 = int(parse_model_config(opt.models_def[3])[0]['channels'])
    ...
    fakenight_loader_city_street = torch.utils.data.DataLoader(fakenight_train_city_street, batch_size=opt.batch_size, shuffle=False, pin_memory=True, collate_fn=fakenight_train_city_street.collate_fn)
    fakenight_loader_highway = torch.utils.data.DataLoader(fakenight_train_highway, batch_size=opt.batch_size, shuffle=False, pin_memory=True, collate_fn=fakenight_train_highway.collate_fn)
    fakenight_loader_residential = torch.utils.data.DataLoader(fakenight_train_residential, batch_size=opt.batch_size, shuffle=False, pin_memory=True, collate_fn=fakenight_train_residential.collate_fn)

    # Init the dataloader for each cluster in night domain
    night_loader_city_street = torch.utils.data.DataLoader(night_train_city_street, batch_size=opt.batch_size, shuffle=False, pin_memory=True, collate_fn=night_train_city_street.collate_fn)
    night_loader_highway = torch.utils.data.DataLoader(night_train_highway, batch_size=opt.batch_size, shuffle=False, pin_memory=True, collate_fn=night_train_highway.collate_fn)
    night_loader_residential = torch.utils.data.DataLoader(night_train_residential, batch_size=opt.batch_size, shuffle=False, pin_memory=True, collate_fn=night_train_residential.collate_fn)
                    ...
                    if tgt_images.shape[0] == batch_size:
                        ...
                        if opt.update_step == "alternating":
                            if opt.modularization == 1:
                                ...
                                y_1_target, y_2_target, y_3_target = D_1(f1_target.detach()), D_2(f2_target.detach()), D_3(f3_target.detach())
                        ...
                        if opt.modularization == 1:
                            ...
                            y_1_target, y_2_target, y_3_target = D_1(f1_target), D_2(f2_target), D_3(f3_target)
                            Ld_1 = bce(y_1_source, torch.FloatTensor(y_1_source.data.size()).fill_(0).cuda())+bce(y_1_target, torch.FloatTensor(y_1_target.data.size()).fill_(1).cuda())
                            Ld_2 = bce(y_2_source, torch.FloatTensor(y_2_source.data.size()).fill_(0).cuda())+bce(y_2_target, torch.FloatTensor(y_2_target.data.size()).fill_(1).cuda())
                            Ld_3 = bce(y_3_source, torch.FloatTensor(y_3_source.data.size()).fill_(0).cuda())+bce(y_3_target, torch.FloatTensor(y_3_target.data.size()).fill_(1).cuda())
                            Ld = Ld_1 + Ld_2 + Ld_3
